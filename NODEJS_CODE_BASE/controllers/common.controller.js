...
module.exports = {
  ...
  getStateList: (req, res) => {
    ...
    state_model.find({ is_active: true }).exec((err, data) => {
  ...
  removeCity: (req, res) => {
    city_model.remove({ _id: req.params.cityId }, (err, result) => {
      ...
      res
        .status(200)
        .json({ message: "City removed successfully", data: result });
    });
  },
  ...
  checkemailAvailability: (req, res) => {
    ...
    users.find({ email: email }, (err, result) => {
      if (err) res.status(400).send(err);
      else if (result.length > 0) res.status(200).json({ response: true });
