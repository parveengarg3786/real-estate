  ...
  getSingleProperty: async (req, res) => {
    try {
      ...
      if (result && result.images.length) {
        files = await gfs.files
          .find({ filename: { $in: result.images } })
          .toArray();
      }
  ...
  markAsSold: async (req, res) => {
    try {
      ...
      if (result && result.nModified == 1)
        res
          .status(200)
          .json({ result, message: "Property has been updated Successfully" });
