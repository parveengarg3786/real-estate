...
export class PropertyNewComponent implements OnInit {
  ...
  getPropertyTypeList() {
    ...
    this.commonService.getPropertyTypeList().subscribe(result => {
      // console.log(result);
      this.propertyTypeList = ["residential", "commercial", "agricultural"];
  ...
  submitForm(data) {
    ...
    this.commonService.togglePageLoaderFn(true);
    this.http.post(environment.BASE_URL + "/property/new", imageData).subscribe(
      result => {
        console.log({ result });
        ...
        if (data && data["slug"]) {
          ...
          this.commonService.changeHeaderMessage({
            type: "danger",
